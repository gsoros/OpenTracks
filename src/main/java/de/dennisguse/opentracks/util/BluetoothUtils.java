...
public class BluetoothUtils {
    ...
    public static final List<UUID> HEART_RATE_SUPPORTING_DEVICES = Collections
            .unmodifiableList(Arrays.asList(BluetoothUtils.HEART_RATE_SERVICE_UUID,
                    // Devices that support HEART_RATE_SERVICE_UUID, but do not announce
                    // HEART_RATE_SERVICE_UUID in their BLE announcement messages (during device
                    // discovery).
    ...
    public static final UUID CYCLING_SPEED_CADENCE_MEASUREMENT_CHAR_UUID = new UUID(0x2A5B00001000L,
            0x800000805f9b34fbL);
     ...
     */
    public static SensorDataCycling.CadenceAndSpeed parseCyclingCrankAndWheel(String address, String sensorName,
        ...
        if (hasWheel && valueLength - index >= 6) {
            long wheelCount = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT32, index);
            index += 4;
        ...
        if (hasCrank && valueLength - index >= 4) {
            int crankCount = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT16, index);
            index += 2;
